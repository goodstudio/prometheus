...
func funcAvgOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
	...
	// calculation without help of Kahan summation (but this should change,
	// see https://github.com/prometheus/prometheus/issues/14105 ). For
	// floats, we improve the accuracy with the help of Kahan summation. For
	// a while, we assumed that incremental mean calculation combined with
	// Kahan summation (see
	...
	return aggrOverTime(vals, enh, func(s Series) float64 {
		...
		for i, f := range s.Floats[1:] {
			...
			q := count / (count + 1)
			mean, kahanC = kahanSumInc(f.F/(count+1), q*mean, q*kahanC)
		}
		if incrementalMean {
			return mean + kahanC
		}
		count := float64(len(s.Floats))
